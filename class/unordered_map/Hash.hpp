...
struct Iterator{
    using Node = HashNode<pair<K,K>>;
    using Self = Iterator<K,Ref,Ptr,KofT,hash>;

    Iterator(const Hash<K,pair<K,K>,KofT,hash>* table, shared_ptr<Node> node)
    ...
    const Hash<K, pair<K,K>, KofT, hash>* hash_table; //用来寻找下一个节点
};
...
public:
    using HashIterator = Iterator<K, V&, V*, KofT, hash>;
    using ConstHashIterator = Iterator<K, const V&, const V*, KofT, hash>;
public:
