...
public:
    ...
    RBTree() = default;
    pair<bool, iterator> insert(const V& kv);
...
template<typename K, typename V, typename KeyOfValue>
pair<bool, typename RBTree<K, V, KeyOfValue>::iterator> 
RBTree<K, V, KeyOfValue>::insert(const V& kv){
    if(_root == nullptr){
        _root = new Node(kv);
        _root->_col = BLACK;
        return {true, iterator(_root, _root)};
    }
    KeyOfValue kov;
    ...
    while(cur){
        ...
        if(kov(kv) > kov(cur->_data)){
            ...
            return {false, iterator(cur, _root)};
        }
    }
    ...
    _root->_col = BLACK;
    return {true, iterator(new_node, _root)};
}
